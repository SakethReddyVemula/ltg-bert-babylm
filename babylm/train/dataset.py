...
class Dataset(Dataset):
    def __init__(self, file, offset: int, n_gpus: int, tokenizer, seq_length=512, mask_p=0.15, short_p=0.1, random_p=0.1, keep_p=0.1):
        ...
        for i, segment in enumerate(open(file, "r")):
            ...
            # Directly tokenize the line using the tokenizer
            segment = self.tokenizer.encode(segment.strip()).tokens
            segment = [self.tokenizer.token_to_id(token) for token in segment]

            # Ensure the length of the segment is within the allowed sequence length
            # assert len(segment) <= seq_length - 2, " ".join(map(str, segment))
            if len(segment) > seq_length - 2:
                segment = segment[:seq_length - 2]

            # Append the token IDs to self.segments
            self.segments.append(segment)
    ...
    def __getitem__(self, index):
        ...
        if tokens is None: ###
            print(f"Error at index {index}: tokens is None") ###
