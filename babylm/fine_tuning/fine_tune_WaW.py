...
def load_data(file_path, is_test=False):
    df = pd.read_csv(file_path, delimiter='\t', header=None, 
                     names=['word_a', 'word_b', 'label'], skiprows=1)
    if not is_test:
...
def main():
    ...
    # Prepare datasets
    train_inputs = prepare_data(train_df.word_a.values, train_df.word_b.values, 
                              train_df.label.values, tokenizer)
    val_inputs = prepare_data(val_df.word_a.values, val_df.word_b.values, 
                            val_df.label.values, tokenizer)
    test_inputs = prepare_data(test_df.word_a.values, test_df.word_b.values, 
    ...
    test_df.to_csv(f"{PATH_TO_TEST_DATA}WaW.test.predictions.tsv", 
                   sep='\t', index=False)
